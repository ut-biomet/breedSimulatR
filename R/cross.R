...
#'   nrow = 2
#' )
#' colnames(rawHaplo1) <- sprintf(
#'   fmt = paste0("SNP%0", 2, "i"),
#'   1:(3 + 4 + 5)
#' )
#' haplo1 <- haplotype$new(
#'   SNPinfo = SNPs,
#'   haplo = rawHaplo1
#' )
...
#'   nrow = 2
#' )
#' colnames(rawHaplo2) <- sprintf(
#'   fmt = paste0("SNP%0", 2, "i"),
#'   1:(3 + 4 + 5)
#' )
#' haplo2 <- haplotype$new(
#'   SNPinfo = SNPs,
#'   haplo = rawHaplo2
#' )
#'
...
#'   nrow = 2
#' )
#' colnames(rawHaplo3) <- sprintf(
#'   fmt = paste0("SNP%0", 2, "i"),
#'   1:(3 + 4 + 5)
#' )
#' haplo3 <- haplotype$new(
#'   SNPinfo = SNPs,
#'   haplo = rawHaplo3
#' )
#'
...
#' )
#'
#' crossToDo <- data.frame(
#'   ind1 = c("Ind 1", "Ind 1", "Ind 2"),
...
makeCrosses <- function(crosses, pop) {
  ...
  if (any(vapply(newInds, function(x) {
    x$name
  }, "character") %in% names(pop$inds))) {
    nameInPop <-
