...
test_that("SNPinfo initialization without link map pos", {
  ...
  SNPcoord <- data.frame(
    chr = rep(mySpec$chrNames, times = nMarkers),
    physPos = physPos,
    SNPid = SNPid
  )
...
test_that("SNPinfo initialization with link map pos", {
  ...
  expect_error(
    {
      SNPs2 <- SNPinfo$new(SNPcoord_2, mySpec)
    },
    NA
  )
  # browser()
...
test_that("SNPinfo errors", {
  ...
  SNPcoord <- data.frame(
    chr = rep(chrNames, times = nMarkers),
    physPos = physPos,
    SNPid = SNPid
  )
  expect_error(
    ...
    paste(
      '"Chromosomes\'names specified in "SNPcoord"',
