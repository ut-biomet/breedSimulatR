...
test_that("selectBV", {
  ...
  expect_error(
    {
      selectedInds <- selectBV(myPop, nSel, myTrait$qtnEff)
    },
    NA
  )
  ...
  expect_error({
    selectedInds <- selectBV(myPop, nSel, c(1, 2, 3, 4))
  })
})
...
test_that("selectWBV", {
  ...
  expect_error(
    {
      selectedInds <- selectWBV(myPop, nSel, myTrait$qtnEff)
    },
    NA
  )
  expect_is(selectedInds, "character")
  ...
  expect_error({
    selectedInds <- selectWBV(myPop, nSel, c(1, 2, 3, 4))
  })
})
